import React from 'react';
import PropTypes from 'prop-types';

import { EntypoCalculator } from 'react-entypo';

// const Icon = ({ size }) => (
//   <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size}>
//     <path fill="currentColor" d="M19.4,9H16V5.6C16,5,15.6,5,15,5s-1,0-1,0.6V9h-3.4C10,9,10,9.4,10,10s0,1,0.6,1H14v3.4c0,0.6,0.4,0.6,1,0.6s1,0,1-0.6V11
//     	h3.4c0.6,0,0.6-0.4,0.6-1S20,9,19.4,9z M7.4,9H0.6C0,9,0,9.4,0,10s0,1,0.6,1h6.8C8,11,8,10.6,8,10S8,9,7.4,9z M7.4,14H0.6
//     	C0,14,0,14.4,0,15s0,1,0.6,1h6.8C8,16,8,15.6,8,15S8,14,7.4,14z M7.4,4H0.6C0,4,0,4.4,0,5s0,1,0.6,1h6.8C8,6,8,5.6,8,5S8,4,7.4,4z"
//     />
//   </svg>
// );
//
// Icon.propTypes = {
//   size: PropTypes.number,
// };
// Icon.defaultProps = {
//   size: 20,
// };
//
export default EntypoCalculator;
